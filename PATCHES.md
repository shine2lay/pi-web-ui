...
- **主栏限额 5 个**：`capTopbarPrimary()` 把超出 `TOPBAR_PRIMARY_MAX` 的可见条目标成
  ...
  **导航也不占额度**（`TOPBAR_ALWAYS_INLINE`：对话/终端/Git/历史/文件/新建对话）——
  第一版把导航一起计数，Git 排第 6 直接被挤进「⋯」，一个导航入口凭空消失比顶栏
  挤一点糟糕得多。额度只约束工具 chip 与插件入口。
- 「⋯」菜单从 `display:none`（原本只在手机上出现）改为桌面也显示。
- 工具组（`.topbar-desktop`）从顶栏**最右**移到**最左**，紧挨着连接状态：右侧留给
  模型选择器与「⋯」，常用入口离视图切换更近，视线不必来回横跳。
