...
**按角色分位置，而不是按数量截断。** 第一版按「前 N 个」砍，结果 Git 这种导航入口
凭空消失 —— 数量不是用户的心智模型，**用途**才是：

- **左边 = 去哪儿**：视图切换（对话/终端/Git）+ 抽屉开关 + **插件页 tab**（一个插件页
  就是一个 tab，随插件增加自然变长，不受任何上限约束）。
- **右边 = 最常用的两个动作**：搜索、新建对话，加上「⋯」。三者包在 `.topbar-right`
  （`margin-left:auto`）里固定靠右。
- **其余一律进「⋯」**：浏览器 / 后台任务 / 设置 / 声音 / 语言 / 主题 / 版本 —— 偶尔碰一次
  的配置项，不值得常驻顶栏。被挤出去的入口排在菜单最上面（点开「⋯」最想找的就是它们）。
- **移除 `host:github`**（内置表、桌面工具组、溢出菜单外链、chip 工厂一并删干净）。
- 顶栏从 `space-between` 改为左对齐；「⋯」从「只在手机显示」改为桌面也显示；
  插件 tab 旁边那个重复的「⋯」删掉（它与右上角的装着同一批条目，点开是空的，
  看起来像坏了）。

用户仍可在设置面板「界面布局」里把任何一条拉回顶栏或藏起来 —— 这里改的只是**默认位置**。
